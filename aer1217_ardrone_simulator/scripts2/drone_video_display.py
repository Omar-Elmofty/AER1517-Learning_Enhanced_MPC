...
from ardrone_autonomy.srv import *       # for receiving rosservice togglecam
...
# OpenCV Image Capture date-time
...
class DroneVideoDisplay(QtGui.QMainWindow):
	...
	def __init__(self):
		...
		# set default cam channel and switch to it
		self.camchannel = FRONT_CAM
		...
		# process images if this boolean is true
		self.processImagesBool = False
		# openCV colour thresholding, note it's written  in B,G,R
		self.colorboundaries = [([80, 50, 200], [250, 200, 255])]
	...
	def RedrawCallback(self):
		if self.image is not None:
			...
			if self.processImagesBool:
				cv_image = self.bridge.imgmsg_to_cv2(self.image, desired_encoding="bgr8")
				height, width, channels = cv_image.shape
				cv_output = np.zeros((height,width,3), np.uint8)
				for (lower, upper) in self.colorboundaries:
					lower = np.array(lower, dtype="uint8")
					upper = np.array(upper, dtype="uint8")
					mask = cv2.inRange(cv_image, lower, upper)
					cv2.bitwise_and(cv_image, cv_image, cv_output, mask=mask)
				img_msg = self.bridge.cv2_to_imgmsg(cv_output, encoding="bgr8")
				self.image_ = QtGui.QPixmap.fromImage(QtGui.QImage(img_msg.data, img_msg.width, img_msg.height, QtGui.QImage.Format_RGB888))
				self.imageBox.setPixmap(self.image_)
			else:
	...
	def SaveImages(self):
		if self.image is not None:
			if self.imageLock.acquire():
				...
				cv2.imwrite('/home/carre/aer1217/ArDrone_images/'+timestring+'.jpeg',(cv_image))
				print "%s.jpeg written to /home/carre/aer1217/ArDrone_images/"%timestring
				self.imageLock.release()
